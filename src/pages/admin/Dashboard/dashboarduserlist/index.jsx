...
const DoctorList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const transformedDoctors = (doctorsResponse.data?.doctors || []).map(
          (doctor) => ({
            id: doctor._id,
            name: doctor.name,
            ...
            totalIncome: doctor.consultationFee
              ? doctor.consultationFee * (doctor.patients || 0)
              : 0,
            monthlyIncome: doctor.consultationFee
              ? Math.floor(
                  (doctor.consultationFee * (doctor.patients || 0)) / 12
                )
              : 0,
        ...
        const transformedAppointments = (
          ...
          slot: appointment.time?.includes("09")
            ? "morning"
            : appointment.time?.includes("14")
            ? "afternoon"
            : "evening",
  ...
  const filteredDoctors = doctorData.filter((doctor) => {
    ...
    console.log(
      `Filtering doctors: search="${searchTerm}", status="${statusFilter}", approval="${approvalFilter}", matches=${
        matchesSearch && matchesStatus && matchesApproval
      }`
    );
  ...
  const filteredUsers = userData.filter((user) => {
    ...
    console.log(
      `Filtering users: search="${searchTerm}", status="${statusFilter}", matches=${
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setDoctorData((prevData) =>
          prevData.map((doctor) =>
            doctor.id === doctorId ? { ...doctor, isActive: true } : doctor
          )
        );
      ...
      <div className="flex items-center p-4 justify-between">
        <div className="relative">
          ...
          <Input
            ...
            placeholder={`Search ${
              activeTab === "doctors"
                ? "doctors by name, email, phone, specialization, or experience"
                : "users by name, phone, location, age, or slot"
            }...`}
          ...
          <IconBtn
            onClick={() => {
              ...
              const fetchData = async () => {
                ...
                setError(null);
                try {
                  const doctorsResponse = await apiInstance.get(
                    "/doctors/admin/all"
                  );
                  const transformedDoctors = (
                    ...
                    totalPatients: doctor.patients || 0,
                    status: doctor.available ? "active" : "inactive",
                    joinDate: new Date(doctor.createdAt)
                      .toISOString()
                      .split("T")[0],
                  }));
                  setDoctorData(transformedDoctors);

                  const appointmentsResponse = await apiInstance.get(
                    "/appointment"
                  );
                  const transformedAppointments = (
                    appointmentsResponse.data?.data || []
                      ...
                      <table className="w-full px-4 table-auto">
                        <thead>
                          <tr className="bg-gray-50">
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[60px] sticky left-0 bg-gray-50 z-10">
                              ID
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[150px] sticky left-[60px] bg-gray-50 z-10">
                              Doctor Name
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[180px]">
                              Email
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[120px]">
                              Phone
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[150px]">
                              Specialization
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[100px]">
                              Experience
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[100px]">
                              Total Income
                        ...
                        <tbody>
                          {paginatedData.length > 0 ? (
                            paginatedData.map((doctor) => (
                              <tr
                                key={doctor.id}
                                className="border-t hover:bg-gray-50 hover:transform hover:translate-x-2 transition-all duration-200"
                              >
                                <td className="p-3 text-gray-500 whitespace-nowrap sticky left-0 bg-white z-10 border-r">
                                  #
                                  {typeof doctor.id === "number"
                                    ? doctor.id
                                    : doctor.id.slice(-4)}
                                </td>
                                <td className="p-3 font-medium whitespace-nowrap sticky left-[60px] bg-white z-10 border-r">
                                  {doctor.name}
                                </td>
                      ...
                      <table className="w-full  table-auto">
                        <thead>
                          <tr className="bg-gray-50">
                            ...
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[80px]">
                              Age
                            </th>
                            <th className="p-3 text-left font-semibold text-gray-600 whitespace-nowrap min-w-[100px]">
                              Slot
